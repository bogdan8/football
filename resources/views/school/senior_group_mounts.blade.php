        ...
        <div class="row">
            <table class="table table-bordered table-browsing-history">
                <div class="browsing-history-block-header">
                    ...
                    ?>
                    @if($day != 1)
                        <a href="/school/senior_group_people/{{$browsing_history[$day-2]->date_visit}}">
                            <
                        </a>
                    @else
                        <a>
                    ...
                    @if($day != 12)
                        <a href="/school/senior_group_people/{{$browsing_history[$day++]->date_visit}}">
                            >
                        </a>
                    @else
                        <a>
                ...
                <tr style="width: 100%;">
                    ...
                    @if(count($browsing_history_mounts->browsing_history_people_day_in_month_school) > 0)
                        @foreach($browsing_history_mounts->browsing_history_people_day_in_month_school[0]->browsing_history_people_day_school as $day)
                            <th>{{$day->day_visit}}</th>
                        @endforeach
                    @endif
                    <th></th>
                ...
                <tbody>
                <?php $i = 1; ?>
                @foreach($browsing_history_mounts->browsing_history_people_day_in_month_school as $people)
                    <?php $count = 0; ?>
